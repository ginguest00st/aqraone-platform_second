...
export default function PanelHeader({
  ...
  return (
    <header
      data-figma-layer="UMKMHeader"
      className="bg-white border-b border-[#E8E6E1] px-6 py-3.5 flex items-center justify-between shrink-0"
      style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.04)" }}
    >
      <div>
        <h1 className="text-[15px] font-bold text-[#1A1714]">{title}</h1>
        {subtitle && (
          <div className="text-[11px] text-[#ABA9A4] mt-0.5">
            {subtitle}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="relative p-2 hover:bg-[#F5F4F1] rounded-[10px] text-[#7C7770]"
        >
        ...
        <div className={`w-9 h-9 ${avatarBg} border-2 border-[#C9A227] rounded-full flex items-center justify-center ${avatarTextColor} font-bold text-[13px]`}>
          {avatarLabel}
        </div>
      </div>
    </header>
